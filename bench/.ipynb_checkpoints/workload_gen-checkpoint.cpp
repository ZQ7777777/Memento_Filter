...
{
    // 检查是否需要允许重复选择
    bool allow_duplicates = n_queries > keys.size();
    
    std::set<uint64_t> chosen_points;
    std::vector<uint64_t> chosen_points_with_duplicates;
    ...
    if (allow_duplicates) {
        // 允许重复，直接生成指定数量的查询
        chosen_points_with_duplicates.reserve(n_queries);
        for (uint64_t i = 0; i < n_queries; ++i) {
            chosen_points_with_duplicates.push_back(keys[rand_keys_distr(gen_random_keys)]);
        }
    } else {
        // 不允许重复，使用原有逻辑
    ...
    // 选择要迭代的容器
    auto process_point = [&](uint64_t point) {
        if (mixed)
        {
    ...
    if (allow_duplicates) {
        for (auto point : chosen_points_with_duplicates) {
            process_point(point);
        }
    } else {
