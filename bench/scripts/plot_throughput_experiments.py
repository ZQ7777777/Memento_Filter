...
def plot_query_exp1(base_dir: Path, fig_dir: Path) -> None:
    ...
    for col_idx, (metric, title) in enumerate(METRICS):
        ...
        rb_qps = float(rb[metric].iloc[0])
        rb_plot_qps = rb_qps / 1e7
        ...
        for ratio in ratios:
            ...
            y_vals = [v / 1e7 for v in y_vals]
            y_series_col.append(y_vals)

            add_ratio_style(ax_top, x_vals, y_vals, float(ratio), markevery)
...
def plot_query_exp1_saturation(base_dir: Path, fig_dir: Path) -> None:
    ...
    for ratio in ratios:
        g = df[df["attack_ratio"] == ratio].sort_values("x")
        x_vals = g["x"].astype(int).tolist()
        y_vals = g["final_saturated_ratio"].astype(float).tolist()
        add_ratio_style(ax, x_vals, y_vals, float(ratio), markevery)

    if not rb.empty:
        ax.axhline(float(rb["final_saturated_ratio"].iloc[0]), color="black", linestyle="--", linewidth=0.8)
...
def plot_query_exp2(base_dir: Path, fig_dir: Path) -> None:
    ...
    for col_idx, (metric, title) in enumerate(METRICS):
        ...
        for ratio in ratios:
            ...
            y_vals = [v / 1e7 for v in g[metric].tolist()]
            y_series_col.append(y_vals)
            ax.plot(
                g["m_exp"].tolist(),
                y_vals,
        ...
        if not rb.empty:
            ...
            rb_vals = [v / 1e7 for v in g_rb[metric].tolist()]
...
def plot_query_exp12_combined(base_dir: Path, fig_dir: Path) -> None:
    ...
    for col_idx, (metric, title) in enumerate(METRICS):
        ax = axes[0, col_idx]
        rb_qps = float(q1_rb[metric].iloc[0]) / 1e7
        y_series_col: list[list[float]] = []
...
def plot_insert_exp2(base_dir: Path, fig_dir: Path) -> None:
...
def main() -> None:
    ...
    ap.add_argument(
        "--figures",
        default="all",
        help="comma-separated ids: q1,q1sat,q2,i1,i2,i3 or all",
    )
    ap.add_argument(
        "--formats",
        default="pdf,svg",
    ...
    if "q2" in selected:
        plot_query_exp2(base_dir, fig_dir)
    if "i1" in selected:
        plot_insert_exp1(base_dir, fig_dir)
    if "i2" in selected:
        plot_insert_exp2(base_dir, fig_dir)
    if "i3" in selected:
