#ifndef CUCKOO_FILTER_FRAPerfectCF_H_
#define CUCKOO_FILTER_FRAPerfectCF_H_
#pragma once

#include <assert.h>
#include <algorithm>

#include "../include/cuckoofilter/src/debug.h"
#include "../include/cuckoofilter/src/hashutil.h"
#include "../include/cuckoofilter/src/packedtable.h"
#include "../include/cuckoofilter/src/printutil.h"
#include "../include/cuckoofilter/src/singletable.h"

// #include "cuckoofilter.h"
#include "../include/cuckoofilter/src/crc.h"
#include "../include/cuckoofilter/src/max64table.h"
#include "../include/cuckoofilter/src/lchash.h"
#include "../include/cuckoofilter/src/MurmurHash3.h"
#include "../include/cuckoofilter/src/cuckoofilter.h"
	// template <typename T>  
...
template <typename ItemType, 
          size_t totalbits = 64, size_t cntSize = 1, size_t recency = 1>
class FRAPerfectCF {
  ...
  inline void GenerateIndexTagHash64(const ItemType& item, size_t* index, uint64_t* tag) const {
    ...
    *index = hash >> (bits_per_item - 1 - cntSize - recency);
    *tag = hash & ((1ULL << (bits_per_item - 1 - cntSize - recency)) - 1);
  }
  ...
  inline void GenerateIndexTagHash32(const ItemType& item, size_t* index, uint32_t* tag) const {
    ...
    *index = hash >> (bits_per_item - 1 - cntSize - recency);
    *tag = hash & ((1 << (bits_per_item - 1 - cntSize - recency)) - 1);
  }

  inline size_t AltIndex(const size_t index, const uint64_t tag) const {
    // NOTE(binfan): originally we use:
    // index ^ HashUtil::BobHash((const void*) (&tag), 4)) & table_->INDEXMASK;
    // now doing a quick-n-dirty way:
    // 0x5bd1e995 is the hash constant from MurmurHash2
    return (size_t)(index ^ ((tag& ((1ULL << (bits_per_item - 1 - cntSize - recency)) - 1)) * 0x5bd1e995))& (this->table_->NumBuckets() - 1);
  }

public:
  explicit FRAPerfectCF(const uint64_t maxBytes) {
    size_t assoc = 4;
    size_t min_num_keys = (maxBytes * 8) / (totalbits + 1 + cntSize + recency);
    this->bits_per_item = static_cast<int>(totalbits - std::ceil(std::log2(min_num_keys)) + 3 + cntSize + recency);
    size_t num_buckets = 1ULL << (totalbits + 1 + cntSize + recency - bits_per_item);
    while (num_buckets * assoc * this->bits_per_item / 8 < maxBytes) {
      this->bits_per_item -= 1;
      num_buckets <<= 1;
    }
    if (num_buckets * assoc * this->bits_per_item / 8 > maxBytes) {
      this->bits_per_item += 1;
      num_buckets >>= 1;
    }
    this->num_items_ = 0;
...
Status Adp64(const ItemType &item)  {
  size_t i1, i2;
  uint64_t tag;
  GenerateIndexTagHash64(item, &i1, &tag);
  i2 = AltIndex(i1, tag);
  this->table_->Adp_FRAPCF(i1, i2, tag, cntSize, recency);
  return Ok;
}

Status AddImpl(const size_t i, const uint64_t tag)  {
  size_t curindex = i;
  uint64_t curtag = tag | (1ULL << (bits_per_item-cntSize-recency)) | (1ULL << (bits_per_item-recency));
  uint64_t oldtag;
  for (uint32_t count = 0; count < kMaxCuckooCount; count++) {
    ...
    curtag ^= 1ULL << (bits_per_item-1-cntSize-recency);
  }
  // test();
  victim_.index = curindex;
  victim_.tag = curtag & ((1ULL << (bits_per_item - cntSize - recency)) - 1);
  victim_.used = true;
  return Ok;
}
...
Status Contain64(const ItemType &key) const {  
  ...
  ((tag ^ (1ULL << (bits_per_item-1-cntSize-recency))) == victim_.tag && i2 == victim_.index) );
  if (found || this->table_->FindTagInBuckets_FRAPCF(i1, i2, tag, cntSize, recency)) {
    return Ok;
  } else {
    return NotFound;
  }
...
// }
  ...
  // number of current inserted items;
  size_t MaxSize() const { return table_->SizeInTags(); }
