#ifndef CUCKOO_FILTER_ADAPERFECTCF_H_
#define CUCKOO_FILTER_ADAPERFECTCF_H_
#pragma once
...
template <typename ItemType, 
          size_t totalbits = 64, size_t cntSize = 1>
class AdaPerfectCF {
...
public:
  // explicit AdaPerfectCF(const size_t max_num_keys) {
  //   size_t assoc = 4;
  ...
  // }
  explicit AdaPerfectCF(const uint64_t maxBytes) {
    size_t assoc = 4;
    size_t min_num_keys = (maxBytes * 8) / (totalbits + 1 + cntSize);
    this->bits_per_item = static_cast<int>(totalbits - std::ceil(std::log2(min_num_keys)) + 3 + cntSize);
    size_t num_buckets = 1ULL << (totalbits + 1 + cntSize - bits_per_item);
